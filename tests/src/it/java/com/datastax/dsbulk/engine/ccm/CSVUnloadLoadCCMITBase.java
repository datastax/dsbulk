...
import static com.datastax.dsbulk.tests.utils.CsvUtils.boundStatements;
import static com.datastax.dsbulk.tests.utils.CsvUtils.createIpByCountryTable;
import static com.datastax.dsbulk.tests.utils.CsvUtils.prepareInsertStatement;
import static com.datastax.dsbulk.tests.utils.CsvUtils.truncateIpByCountryTable;
...
abstract class CSVUnloadLoadCCMITBase {
  ...
  CSVUnloadLoadCCMITBase(Session session) {
  ...
  @Test
  void should_load_csv_files() throws Exception {
    MetricsCollectingExecutionListener metrics = new MetricsCollectingExecutionListener();
    MetricsReportingExecutionListener reporter = new MetricsReportingExecutionListener(metrics);
    BulkExecutor executor = getBulkExecutor(reporter, session);
  ...
  @Test
  void should_read_and_write_from_cassandra() throws Exception {
    MetricsCollectingExecutionListener metrics = new MetricsCollectingExecutionListener();
    MetricsReportingExecutionListener reporter = new MetricsReportingExecutionListener(metrics);
    BulkExecutor executor = getBulkExecutor(reporter, session);
