...
public interface LoaderConfig extends Config {
  ...
  default <T> T getInstance(String path) {
    ...
    try {
      ...
      throw new ConfigException.WrongType(origin(), path, "FQCN or short class name", getValue(path).valueType().toString(), e);
    }
  }
  ...
  default URL getURL(String path) {
    ...
    try {
      ...
      try {
        ...
        e1.addSuppressed(e);
        throw new ConfigException.WrongType(origin(), path, "path or URL", getValue(path).valueType().toString(), e1);
      }
    }
  }
    ...
    try {
      ...
      if (matcher.find()) {
        ...
        throw new ConfigException.WrongType(
          origin(), path, "integer or string in 'nC' syntax", getValue(path).valueType().toString());
      }
    }
    return threads;
  }
  ...
  default char getChar(String path) {
    String setting = getString(path);
    if (setting.length() != 1) {
      throw new ConfigException.WrongType(origin(), path, "single character", getValue(path).valueType().toString());
    }
  ...
  default Charset getCharset(String path) {
    ...
    try {
      ...
      throw new ConfigException.WrongType(origin(), path, "valid charset name", getValue(path).valueType().toString(), e);
    }
  }
