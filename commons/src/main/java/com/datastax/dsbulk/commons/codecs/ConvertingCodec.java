...
public abstract class ConvertingCodec<EXTERNAL, INTERNAL> implements TypeCodec<EXTERNAL> {
  ...
  public ConvertingCodec(TypeCodec<INTERNAL> internalCodec, GenericType<EXTERNAL> javaType) {
    this.internalCodec = internalCodec;
  ...
  @Override
  public ByteBuffer encode(EXTERNAL external, ProtocolVersion protocolVersion) {
    INTERNAL value = externalToInternal(external);
    return internalCodec.encode(value, protocolVersion);
  }

  @Override
  public EXTERNAL decode(ByteBuffer bytes, ProtocolVersion protocolVersion) {
