...
public class DCInferringDseLoadBalancingPolicy implements LoadBalancingPolicy, RequestTracker {
  ...
  public DCInferringDseLoadBalancingPolicy(
  ...
  public void init(@NonNull Map<UUID, Node> nodes, @NonNull DistanceReporter distanceReporter) {
    ...
    if (localDc == null) {
        ...
        throw new IllegalStateException(
            String.format(
                "No local DC was provided, but the contact points are from different DCs: %s; "
                    + "please set the local DC explicitly with -localDc, or check your contact points",
                formatContactPointsAndDatacenters(datacenters)));
      }
      if (!datacenters.isEmpty()) {
        localDc = datacenters.values().iterator().next();
        LOG.info("[{}] Inferred local DC from contact points: {}", logPrefix, localDc);
      }
    } else {
      Map<Node, String> badContactPoints = new LinkedHashMap<>();
      for (Node node : contactPoints) {
        String datacenter = node.getDatacenter();
      ...
      if (badContactPoints.entrySet().containsAll(contactPoints)) {
        throw new IllegalStateException(
            String.format(
                "You specified {} as the local DC, but all contact points are from a different DC: %s; "
                    + "please provide the correct local DC with -localDc, or check your contact points",
                formatContactPointsAndDatacenters(badContactPoints)));
      }
      if (!badContactPoints.isEmpty()) {
        LOG.warn(
            "[{}] You specified {} as the local DC, but some contact points are from a different DC: {}; "
                + "please check your contact points",
            logPrefix,
            localDc,
            formatContactPointsAndDatacenters(badContactPoints));
      }
  ...
  private String formatContactPointsAndDatacenters(Map<Node, String> badContactPoints) {
    return badContactPoints.entrySet().stream()
        .map(
            e -> {
              String datacenter = e.getValue();
              Node node = e.getKey();
              return node.getEndPoint() + " -> " + datacenter;
            })
        .collect(joining(", "));
  }
