...
public class LogManager implements AutoCloseable {
  ...
  private FastThreadLocal<Map<URI, List<Range<Long>>>> positionsThreadLocal;
  private List<Map<URI, List<Range<Long>>>> allPositions;
  ...
  public void init() {
    ...
    allPositions = new CopyOnWriteArrayList<>();
    positionsThreadLocal =
        new FastThreadLocal<Map<URI, List<Range<Long>>>>() {
          @Override
          protected Map<URI, List<Range<Long>>> initialValue() {
            Map<URI, List<Range<Long>>> map = new HashMap<>();
  ...
  public void close() throws IOException {
    ...
    if (workflowType == WorkflowType.LOAD) {
      mergeAndPrintPositions();
    }
  }

  private void mergeAndPrintPositions() throws IOException {
    // sort positions by URI
    SortedMap<URI, List<Range<Long>>> sortedPositions = new TreeMap<>();
    for (Map<URI, List<Range<Long>>> positions : allPositions) {
      for (Entry<URI, List<Range<Long>>> entry : positions.entrySet()) {
        sortedPositions.merge(entry.getKey(), entry.getValue(), LogManager::mergePositions);
      }
    }
