...
class NodeComparatorTest {
  ...
  void should_compare_two_hosts(InetSocketAddress a1, InetSocketAddress a2, int expectedResult) {
    // given
    Node host1 = createHostWithAddressAndPort(a1);
    Node host2 = createHostWithAddressAndPort(a2);

    // when
    int compare = new NodeComparator().compare(host1, host2);
  ...
  private Node createHostWithAddressAndPort(InetSocketAddress address) {
    Node h1 = mock(Node.class);
    when(h1.getEndPoint()).thenReturn(new DefaultEndPoint(address));
    return h1;
  }
}
