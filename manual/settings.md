...
* *MULTI_DOCUMENT*: each resource may contain an arbitrary number of successive, independent Json documents to be mapped to records, e.g.:
```json
{/*doc1*/}
{/*doc2*/}
{/*doc3*/}
...
When reading, the connector will validate that the files comply with the mode specified here, and will throw an error if that is not the case.

When writing, the connector will create files in the mode specified here; regardless of the mode being used, each record will be printed in a different line, just as shown in the examples above.
