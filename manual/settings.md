...
Important: the query will be parsed because DSBulk needs to know which bound variables are present in order to correctly map them to fields. The parser used internally is based on CQL 3.4.5 (as used in Apache Cassandra 3.11.1) and is capable to parse all statements defined by that specific grammar version, even complex ones. However, future versions of CQL might introduce new grammar rules that DSBulk's parser will not be able to parse; DSBulk will emit a warning in such situations.
