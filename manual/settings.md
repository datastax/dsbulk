...
For counting, the statement must be a `SELECT` statement; the SELECT clause must comply with the following requirements:
- When counting partitions: the SELECT clause must contain the all table's partition key columns, and nothing else;
- When counting ranges or hosts: the SELECT clause must contain only one `token(...)` selector, e.g. `SELECT token(...) FROM ...`;
- When counting the total number of rows: the SELECT clause can contain any selector.
The statement can optionally contain a token range restriction clause of the form: `token(...) > :start and token(...) <= :end`.
