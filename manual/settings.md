...
For counting, the statement must be a `SELECT` statement and the table's partition key must be present among the selected columns; as for unloading, it can optionally contain a token range restriction clause of the form: `token(...) > :start and token(...) <= :end`.
