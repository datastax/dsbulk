...
#### --executor.throttling.enabled _&lt;boolean&gt;_

Whether or not to enable throttling. Throttling is disabled by default.

Default: **false**.

#### --executor.throttling.rate _&lt;number&gt;_

The rate to apply, in number of operations (both reads and writes) per second.

In `FIXED` mode, this is the constant rate that will be applied; in `DYNAMIC` mode, this is just the initial rate, but actual rates will be obtained
through statistics and used instead of this value as soon as possible.

Batch statements count for as many operations as their number of inner statements.
...
#### --executor.throttling.checkInterval _&lt;string&gt;_

The interval at which the rate should be checked. Relevant only when the throttling mode is `DYNAMIC`, ignored otherwise.

Default: **"15 seconds"**.

#### --executor.throttling.downMultiplier _&lt;number&gt;_

The multiplier to apply to the current rate when adjusting it down. Relevant only when the throttling mode is `DYNAMIC`, ignored otherwise.

Values must be greater than 0 and lesser than 1.

Default: **0.25**.

#### --executor.throttling.maxThreshold _&lt;number&gt;_

The threshold above which the current rate will be adjusted down. Relevant only when the throttling mode is `DYNAMIC`, ignored otherwise.

The meaning of this value depends on the statistics provider:
- For **IN_FLIGHT**: this should be the maximum number of in-flight requests to accept before adjusting the current rate down; note that if `dsbulk.executor.maxInFlight` is also provided, then this value cannot be higher than that.
- For **LATENCY**: this should be the maximum latency in milliseconds to accept before adjusting the current rate down.

This value must be strictly positive.

Default: **512**.

#### --executor.throttling.minThreshold _&lt;number&gt;_

The threshold below which the current rate will be adjusted up. Relevant only when the throttling mode is `DYNAMIC`, ignored otherwise.

The meaning of this value depends on the statistics provider:
- For **IN_FLIGHT**: this should be the minimum number of in-flight requests to accept before adjusting the current rate up.
- For **LATENCY**: this should be the minimum latency in milliseconds to accept before adjusting the current rate up.

This value must be strictly positive.

Default: **1**.

#### --executor.throttling.mode _&lt;string&gt;_

The throttling mode. Valid values are:
- **FIXED**: Applies a constant rate; when this mode is selected, only the `rate` setting must be provided. Other settings in this section are ignored.
- **DYNAMIC**: Applies an adjustable rate with exponential back-off; when this mode is selected, all settings in this section are ignored.

Default: **"FIXED"**.

#### --executor.throttling.statProvider _&lt;string&gt;_

The statistics provider to use. Relevant only when the throttling mode is `DYNAMIC`, ignored otherwise.

Valid values are:
- **IN_FLIGHT**: Uses the current number of in-flight requests as a source of statistics; this provider is believed to provide the most reliable statistics.
- **LATENCY**: Uses the recorded latencies at the 99.9 percentile as a source of statistics.

Default: **"IN_FLIGHT"**.

#### --executor.throttling.upMultiplier _&lt;number&gt;_

The multiplier to apply to the current rate when adjusting it up. Relevant only when the throttling mode is `DYNAMIC`, ignored otherwise.

Values must be greater than 0 and lesser than 1.

Default: **0.1**.

#### --executor.throttling.warmUpPeriod _&lt;string&gt;_

The warm-up period. The rate will only be checked once this period is elapsed. Relevant only when the throttling mode is `DYNAMIC`, ignored otherwise.
